...
function ReverseProxy(opts) {
  ...
  function websocketsUpgrade(req, socket, head) {
    ...
    _this._getTarget(src, req).then(function (target) {
      ...
      if (target) {
        ...
        respondNotFound(req, socket, 404);
      }
    });
  }
  ...
  function handleProxyError(err, req, res) {
    ...
    if (err.code === 'ECONNREFUSED') {
      respondNotFound(req, res, 503);
    } else if (!res.headersSent) {
      respondNotFound(req, res, 500);
    }
...
var respondNotFound = function (req, res, code) {
  res.statusCode = code;
  res.write('Not Found');
  res.end();
};
